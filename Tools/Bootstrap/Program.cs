...
// Copies vanilla DREDGE DLLs from the NuGet cache (populated by `dotnet restore`
// of this csproj) into ModUnity/Assets/Plugins/Dredge/. Run on a fresh clone via
// the root bootstrap.cmd (or directly: `dotnet run --project Tools/Bootstrap`).
//
// The hash + package mapping must stay in sync with DREDGE_DLLS in BuildArtifacts.cs.

const string PluginsRelDir = "ModUnity/Assets/Plugins/Dredge";

var dlls = new (string Name, string Package, string Version, string Subpath, string Sha256)[]
{
...
string modRepoRoot = FindModRepoRoot();
